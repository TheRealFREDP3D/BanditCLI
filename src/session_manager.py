...
class SessionManager:
    ...
    def _load_sessions(self) -> None:
        ...
        Reads the sessions file and populates in-memory session cache.
    ...
    def _migrate_legacy_file(self, legacy_file: Path) -> None:
        ...
        Args:
            legacy_file: Path to the legacy sessions file.
        """
        try:
            ...
            # Treat as plain text file - unlikely for sessions but handle gracefully
            try:
                # For sessions, plain text format is not expected,
                # so we create an empty sessions file
                new_data = {
                    "sessions": {},
                    "active_session_id": None,
                    "last_updated": datetime.now().isoformat(),
    ...
    def _save_sessions(self) -> None:
        ...
        try:
            ...
            data = {
                "sessions": {
                    session_id: session.to_dict() for session_id, session in self.sessions.items()
                },
        ...
        with self.lock:
            ...
            for session_id, session in self.sessions.items():
                ...
                if (
                    ...
                    and session.last_used.timestamp() < cutoff_time
                ):
