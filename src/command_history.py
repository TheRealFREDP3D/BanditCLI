...
class CommandHistory:
    ...
    def __init__(self, max_history_size: int = 1000) -> None:
        ...
        self.history_file = self.bandit_dir / "history"
    ...
    def filter_by_prefix(self, prefix: str) -> List[str]:
        ...
        with self.lock:
            ...
            return [cmd for cmd in reversed(self.history) 
        ...
        with self.lock:
            return {
                ...
                'last_updated': datetime.fromtimestamp(
                    self.history_file.stat().st_mtime
                ).isoformat() if self.history_file.exists() else None
            }
        ...
        try:
            with self.lock:
                if format_type == 'json':
                    ...
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(f"# BanditCLI Command History Export\n")
